...
def do_test(args, cfg, config, model, logger_1, iteration):
    ...
    train_data = process(train_dset, model, config)

    #print('train_data:', train_data)

    val_data = process(val_dset, model, config)
    ...
    model_classifier = Classifier(num_classes=num_classes, dim=256).cuda()
...
def do_knn(knn_dataset, cfg, config_path, root_dir, model, iteration, logger_1):
    ...
    for i, dataset_name in enumerate(all_dataset_names):
        ...
        train_data = process(knn_dataset[i * 2], model, config)
        val_data = process(knn_dataset[i * 2 + 1], model, config)
...
def do_train(cfg, model, resume=False):
    ...
    for data in metric_logger.log_every(
        ...
        if cfg.evaluation.eval_period_iterations > 0 and (iteration + 1) % cfg.evaluation.eval_period_iterations == 0:
            ...
            max_acc = do_test(args, cfg, config, model, logger_1, iteration)
            torch.cuda.synchronize()
