...
def do_test(args, cfg, config, model, logger_1):

    if distributed.is_main_process():

        root_dir = '/mnt/data/aim/liyaxuan/projects/project2/'
        config_path = root_dir + 'configs/config_classification.json'
        ...
        backbonemodel = SSLMetaArch(cfg, test_flag=True).to(torch.device("cuda"))
        #backbonemodel.prepare_for_distributed_training()

        #print('after')

        model_weights = copy.deepcopy(model.state_dict())
        backbonemodel.load_state_dict(model_weights)
        print('value_check:', model.student["backbone"].value())
        dset = BBP_dataset(data_path=data_path, keep_node=test_keep_node)

        model_classifier = Classifier(backbone=backbonemodel.student.backbone, num_classes=num_classes, dim=32).cuda() # Assuming backbone.student outputs feature vectors
        ...
        for epoch in range(num_epochs):
            ...
            print('value_check:' + epoch, model.student["backbone"].value())
            ...
            for feat, adj, label in train_loader:
                ...
                sum_num += len(feat)
            #print('len:', len(train_loader), running_loss)
            print(f"Epoch {epoch+1}, Loss: {running_loss/len(train_loader)} Acc: { ACC * 100.0 / sum_num}%")
            logger_1.info('Epoch {} | Train: Loss {:.4f} | Acc {:.2f}%'.format(epoch+1, running_loss/len(train_loader), ACC * 100.0 / sum_num))
            ...
            logger_1.info('Epoch {} | Val: Loss {:.4f} | Acc {:.2f}%'.format(epoch+1, running_loss/len(val_loader), ACC * 100.0 / sum_num))
...
def save_checkpoint(cfg, model, loss, iteration):
    ...
    if distributed.is_main_process():
        ...
        if os.path.exists(eval_base_dir):
            ...
            if len(subdirs) > 5:
                highest_loss_dir = max(subdirs, key=lambda x: float(x))  # get the directory with highest loss
...
def do_knn(knn_dataset, cfg, config_path, root_dir, model, iteration, logger_1):

    def process(dset, mode, cfg, model, config, config_path, data_dir):
    ...
    if distributed.is_main_process():
        ...
        for i, dataset_name in enumerate(all_dataset_names):
            ...
            train_data = process(knn_dataset[i * 2], 'train', cfg, model, config, config_path, dataset_path)
            val_data = process(knn_dataset[i * 2 + 1], 'val', cfg, model, config, config_path, dataset_path)
            ...
            logger_1.info('Iteration {} | Dataset: {} |Train: Acc {:.2f}%'.format(iteration, dataset_name, max_acc))
...
def do_train(cfg, model, resume=False):
    ...
    dir_name = '/mnt/data/aim/liyaxuan/projects/project2/benchmark_datasets/'
    config_path = '/mnt/data/aim/liyaxuan/projects/project2/configs/config.json'
    ...
    for i, dataset_name in enumerate(all_dataset_names):
        dataset_path = dir_name + dataset_name + '/'
        knn_dataset[i * 2] = NeuronMorpho(root='', extra='', keep_node=cfg.dataset.node_num, split=NeuronMorpho.Split['TRAIN'], config_path = config_path, mode='train', inference=True, data_path = dataset_path)
        knn_dataset[i * 2 + 1] = NeuronMorpho(root='', extra='', keep_node=cfg.dataset.node_num, split=NeuronMorpho.Split['TRAIN'], config_path = config_path, mode='val', inference=True, data_path = dataset_path)
    
    node_num = cfg.dataset.node_num
    ...
    for data in metric_logger.log_every(
        ...
        do_test(args, cfg, config, model, logger_1)
        torch.cuda.synchronize()
        acc_mean = do_knn(knn_dataset, cfg, config_path, knn_dir, model, iteration, logger_1)
        torch.cuda.synchronize()
        if cfg.evaluation.eval_period_iterations > 0 and (iteration + 1) % cfg.evaluation.eval_period_iterations == 0:
            ...
            if len(acc_list) < 5:
                ...
                save_checkpoint(cfg, model, root_dir, acc_mean, iteration)
                torch.cuda.synchronize()

            elif acc_mean < np.max(acc_list):
                delete_acc = np.max(acc_list)
                acc_list = [acc for acc in acc_list if acc != delete_acc]
                acc_list.append(now_acc)
                save_checkpoint(cfg, model, now_acc, iteration)
                torch.cuda.synchronize()
