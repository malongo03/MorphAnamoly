...
def read_swc(file_name, columns="NOT_USED", numeric_columns="NOT_USED"):
    ...
    try:
        with open(file_name, "r", errors='ignore') as f:
            ...
            for line in f:
                ...
                import copy
                line_tmp = copy.deepcopy(line)
                st = 0
                while line_tmp[st] == ' ':
                    st += 1
                if st > len(line_tmp) -2:
                    continue
    ...
    #if bug == 1:
        #print('compartments', Morphology(compartment_list=compartments))
...
class Morphology(object):
    ...
    def __init__(self, compartment_list=None, compartment_index=None):
        ...
        '''
        n_now_compartment = len(compartment_list)
        for i in range(n_now_compartment, 1500):
            none_compartment = {'id': i, 'type': -1, 'x':0.0, 'y':0.0, 'z':0.0, 'radius':0.0, 'parent':0, 'tree_id': -1}
            compartment_list.append(none_compartment)
        '''
    ...
    def _decode(self):
        ...
        for node in self.compartment_list:
            now_decode = [node['id'], node['type'], node['x'], node['y'], node['z'], node['radius'], node['parent']]
            result_list.append(now_decode)
